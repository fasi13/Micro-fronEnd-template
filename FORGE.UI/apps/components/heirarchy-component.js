...
(function () {
    ...
    function controller($http, $cookies, $scope, apiService, $timeout, tree, shareHierarchyData, update_brandingContent, getChild, collapseHierarchy, logoimg, updateConfigNode, $rootScope) {
        var self = this;
        ...
        shareHierarchyData.token = JSON.parse($cookies.get('profile'))._token;

        apiService.get(baseApplicationUrl,
                                 HierarchyLoadCompleted,
                                 HierarchyLoadFailed, shareHierarchyData.token);
        ...
        function HierarchyLoadCompleted(result) {
            ...
            if (result.status == 200) {
                ...
                shareHierarchyData.get_root($scope.root);
                ...
                alert('Hierarchy load failed' + result.error);
            }

        }

        function HierarchyLoadFailed(result) {
            alert('Hierarchy load failed');
        }
        self.showTree = true;
        ...
        $scope.search = function (text) {
            ...
            if (text.length > 2) {
                ...
                apiService.get(const_APIUrl + "/applications?keyword=" + text, SearchResultSuccess, SearchResultFail, shareHierarchyData.token);
            }
        ...
        $scope.$on('childUpdated', function () {
            ...
            for (var i = 1; i < $scope.nodePath.length; i++) {
                ...
                if (containNode(appId)) {
                    ...
                    if (!newNode.IsApplicationGroup) {
                        newNode.click = "autoClick";
                        shareHierarchyData.clickedNode(newNode, $scope.root);
                        $scope.toggleRootNode(newNode.unique);
        ...
        $scope.$on('onApplicationSelected', function () {
            ...
            $scope.clicked_node = shareHierarchyData.node;
            $scope.parent = shareHierarchyData.parent;
        ...
        function HierarchyBrandingAttributesLoadCompleted(result) {
            ...
            if (result.status == 200) {
                ...
                alert('Branding Contents load failed' + result.error);
            }
        ...
        function getHierarchyBrandingAttributes(contentURL, attribute) {
            ...
            var value;
            apiService.getBrandingData(getAttributeURL, shareHierarchyData.token).then(function (brandingData) {
                var bradingDataCopy = {};
        ...
        $scope.config_breadcrumb = function (currentNode) {
            ...
            if (currentNode != undefined) {
                ...
                shareHierarchyData.node = currentNode;
            }
            else {
    ...
    angular
       ...
       .config(function (ivhTreeviewOptionsProvider) {
        ...
        .component('heirarchyComponent', {
            ...
            controller: ['$http', '$cookies', '$scope', 'apiService', '$timeout', 'tree', 'shareHierarchyData', 'update_brandingContent', 'getChild', 'collapseHierarchy', 'logoimg', 'updateConfigNode', '$rootScope', controller],
            controllerAs: 'hc'
        });
